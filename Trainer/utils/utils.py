'''
Code taken from https://github.com/mahmoodlab/Patch-GCN
'''
import pickle
import torch
import numpy as np
...
def collate_MIL_survival_hvt(batch):
...
def collate_MIL_survival_cluster(batch):
    ...
    return [img, cluster_ids, label, event_time, c, case_id]
...
def collate_MIL_survival_graph(batch):
    ...
    graph = batch[0][0]
    assert isinstance(graph, torch_geometric.data.Batch)
    label = torch.LongTensor([item[1] for item in batch])
    event_time = np.array([item[2] for item in batch])
    c = torch.FloatTensor([item[3] for item in batch])
    case_id = [item[4] for item in batch]
    return [graph, label, event_time, c, case_id]
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    if mode == 'graph':
        ...
        collate = collate_MIL_survival_hvt
    else:
